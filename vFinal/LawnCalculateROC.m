...
%threshold ranges
TOVThreshold = [0:0.09:0.054 , 0.01:0.1:0.41, 0.55:0.05:0.95];
...
for videoNo = 1:2
    ...
    for j = 1:length(TOVThreshold)
        ...
        %calculate tp and fp rates (the coefficient 1/2 is basically for
        %averaging)
        tp_rate(j+1) = tp_rate(j+1) + (1/2) * sum((alarms == 1) & (alarmsDetected == 1)) / sum(alarms);
        fp_rate(j+1) = fp_rate(j+1) + (1/2) * sum((alarms == 0) & (alarmsDetected == 1)) / sum(alarms == 0);
...
%plot roc
graph1 = plot(fp_rate,tp_rate,'-b'); xlabel('FPR');ylabel('TPR');
set (graph1,'LineWidth',3);
auc1 = -trapz(fp_rate,tp_rate)
