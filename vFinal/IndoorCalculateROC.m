...
%threshold ranges
TOVThreshold = [0:0.005:0.02 , 0.03:0.01:0.2];
...
for videoNo = 3:8
    ...
    for j = 1:length(TOVThreshold)
        ...
        %calculate tp and fp rates (the coefficient 1/6 is basically for
        %averaging)
        tp_rate(j+1) = tp_rate(j+1) + (1/6) * sum((alarms == 1) & (alarmsDetected == 1)) / sum(alarms);
        fp_rate(j+1) = fp_rate(j+1) + (1/6) * sum((alarms == 0) & (alarmsDetected == 1)) / sum(alarms == 0);
...
%plot roc
graph2 = plot(fp_rate,tp_rate,'-g'); xlabel('FPR');ylabel('TPR');
set (graph2,'LineWidth',3);
auc2 = -trapz(fp_rate,tp_rate)
